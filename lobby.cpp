...
void Lobby::set_font(QWidget *widget, QString p_identifier)
{
  ...
  if (use)
  {
    ...
    bool bold = (bool)ao_app->get_font_size(
        p_identifier + "_bold", design_file); // is the font bold or not?
    bool center = (bool)ao_app->get_font_size(
        p_identifier + "_center", design_file); // should it be centered?
