...
void Courtroom::create_widgets()
{
  ...
  load_shouts(); // Reads from theme, deletes old shouts if needed and creates
                 // new ones
...
void Courtroom::connect_widgets()
{
  ...
  connect(ao_config, SIGNAL(theme_changed(QString)), this,
...
void Courtroom::set_widgets()
{
  ...
  // Set the default values for the buttons, then try and determine if they
  // should be replaced by images
...
void Courtroom::set_font(QWidget *widget, QString p_identifier)
{
  ...
  int bold = ao_app->get_font_size(p_identifier + "_bold",
                                   design_file); // is the font bold or not?
