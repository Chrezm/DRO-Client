...
void Courtroom::enter_courtroom(int p_cid)
{
  ...
  select_emote(
      0); // by doing this, we can also automatically tick always pre if needed
...
void Courtroom::on_chat_return_pressed()
{
  ...
  {
    // Do note that sfx_names does NOT have the sfx names with [X] due to
    // selection, they are only "visible" on the text of the individual
    // QListWidgetItem
...
{
  QListWidgetItem *old_sfx =
      (current_sfx_id == -1) ? nullptr : (ui_sfx_list->item(current_sfx_id));
