...
void Courtroom::handle_music_anim()
{
  ...
  float speed =
      static_cast<float>(ao_app->get_font_size("music_name_speed", file_b));
...
void Courtroom::handle_chatmessage_2() // handles IC
{
  ...
  if (f_color == "")
    f_color = "rgb(" +
              ao_app->read_theme_ini("showname_color", "courtroom_fonts.ini") +
              ")";

  // is the font bold or not? // taken directly from function up there lol //
  // kinda hacky
...
void Courtroom::handle_chatmessage_3()
{
  ...
  // Check for any of 9 possible ways that showname images are recognized.
  // 3 extensions
  QVector<QString> exts = {".png", ".jpg", ".bmp"};

  // 3 places (in order)
  // 1. Variant folder in theme folder in character folder
  // 2. Theme folder in character folder
  // 3. Character folder
  QStringList subdirectories = {
      "themes/" + ao_app->get_theme() + "/" + ao_app->get_theme_variant() + "/",
      "themes/" + ao_app->get_theme() + "/",
      "",
  };

  QString ext, path;
  for (QString subdirectory : subdirectories)
  {
    path = ao_app->get_character_path(f_char) + subdirectory + "showname";
    ext = file_exists(path, exts);
    if (!ext.isEmpty())
      break;
  }

  if (!ext.isEmpty() && !chatmessage_is_empty &&
      ao_app->read_theme_ini("enable_showname_image", cc_config_ini) == "true")
  {
    ui_vp_showname->hide();
    ui_vp_showname_image->set_image_from_path(path + ext);
    ui_vp_showname_image->show();
  }
  else
  {
...
void Courtroom::update_ic_log(bool p_reset_log)
{
  ...
  name_format.setForeground(
      ao_app->get_color("chatlog_showname_color", fonts_ini));

  QTextCharFormat line_format = ui_ic_chatlog->currentCharFormat();
  line_format.setFontWeight(QFont::Normal);
  line_format.setForeground(
      ao_app->get_color("chatlog_message_color", fonts_ini));
  ...
  for (record_type_ptr record : records_to_add)
  {
    ...
    {
      cursor.insertText(
          record->name + (m_chatlog_newline ? QString(QChar::LineFeed) : ": "),
          name_format);
...
void Courtroom::append_ic_text(QString p_name, QString p_line, bool p_system)
{
  // record new entry
  m_ic_records.append(
      std::make_shared<record_type>(p_name, p_line, "", p_system));
...
void Courtroom::chat_tick()
{
  ...
  if (ao_app->read_theme_ini("enable_vp_message_outline", cc_config_ini) ==
      "true")
  ...
  else
  {
             ...
             "true")
    {
      ...
      for (const auto &col : f_vec)
      {
        if (f_character == col[0].trimmed()[0] &&
            m_string_color != col[1].trimmed())
        {
